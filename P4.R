...
w <- which(fab$LifeSt %in% c("mar", "mar,par"))
fab <- fab[w,]
aoo <- aoo[w,]

# Restrict data to coastal and bottom-dwelling species
# (This is based on ecosystem codes defined by "Nature in Norway", according to 
# the EcoSyst framework, see Halvorsen et al. 2020 [doi:10.1111/geb.13164])
...
# (4) Is any AOO greater than the area of the Norwegian Economic Zone?
w <- which(aoo$high > 900000)
if (length(w)) {
  print(aoo[w,])
} else {
  cat("Everything is fine.\n")
...
M <- 900000  # maximum possible area
...
{ # Output of the results
  cat("P2 is " %+% round(mean(apply(AOO, 1, sum))) %+% " ± " %+%
