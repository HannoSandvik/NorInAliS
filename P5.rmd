...
for (i in fab$Name) {
  ...
  ANO$fa_registrert[w] <- ANO$fa_registrert[w] + 1
}
```
...
¤¤¤
```{r}
table(ANO$fa_total_dekning > 0, ANO$MES, useNA="always")
```
...
```{r}
table(AlienSpecies)
table(ANO$fa_total_dekning > 0, ANO$fa_registrert, useNA="always")
```

```{r}
ANO$fa_total_dekning[ANO$fa_registrert > 0] <- sapply(ANO$fa_total_dekning[ANO$fa_registrert > 0], max, 0.1, na.rm = TRUE)
```
